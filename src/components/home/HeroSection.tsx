...
export const HeroSection = ({ onQuickOrder, onMenuClick }: HeroSectionProps) => {
  ...
  return (
    <section className="relative overflow-hidden bg-secondary" dir={direction}>
      {/* Decorative elements matching reference */}
      <div className="absolute -top-12 -right-12 w-36 h-36 bg-primary/10 rounded-full" />
      <div className="absolute top-20 -left-8 w-20 h-20 bg-juice-pink/20 rounded-full" />
      <div className="absolute bottom-8 right-8 w-12 h-12 bg-primary/15 rounded-full" />

      <div className="relative px-5 pt-10 pb-8">
        {/* Top bar: Greeting + Logo */}
        <div className="flex items-center justify-between mb-8 animate-fade-in">
          <div>
            <p className="text-foreground/60 text-sm font-medium">{greeting}</p>
        ...
        <div className="space-y-4 animate-slide-up">
          <div className="inline-flex items-center gap-2 bg-primary/15 rounded-full px-4 py-2">
          ...
          <h1 className="text-3xl font-display font-bold text-foreground leading-tight">
            {t("mrJuice")}{" "}
            <span className="text-primary">{t("freshDrinks")}</span>
          </h1>
          ...
          <div className="flex gap-3 pt-2">
            <Button 
              ...
              className="flex-1 group rounded-full h-14"
            >
              {t("orderNow")}
            ...
            <Button 
              ...
              variant="outline"
              onClick={onMenuClick}
              className="rounded-full h-14 border-2 border-primary text-primary hover:bg-primary hover:text-primary-foreground"
            >
              {t("menu")}
            </Button>
          </div>
